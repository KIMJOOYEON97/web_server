...
public class TestPerson2Servlet extends HttpServlet {
	...
	@Override          //톰캣이 전달해준 request response
	protected void doPost(HttpServletRequest request, HttpServletResponse response) 
		throws ServletException, IOException {
		//0. encoding 선언
		request.setCharacterEncoding("utf-8"); //이거 꼭 해주기
		//1. 사용자입력값 변수에 담기
		String name = request.getParameter("name");
        String color =request.getParameter("color");
        String animal =request.getParameter("animal");
        //체크박스 복수개의 답일 경우 getParameterValues
        ...
        //key:String, value:Object   저장    	Object(모든 자바 속성 담을 수 있음)
        ...
        reqDispatcher.forward(request, response); //request response전달(위임)해서jsp에서도 접근가능
//        =>jsp도 servlet이다.
//        reqDispatcher.forward(request, response,recommendation); //request response전달해서jsp에서도 접근가능
